...
| Item           | Description                                                                                                                                                                                                                                                                                                                                                 |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Project Name   | Микросервис журналирования статусов движения товаров в маркетплейсе                                                                                                                                                                                                                                                                                         |
| Project Vision | Обеспечить единый журнал статусов движения товаров по всей цепочке поставок: от поставщика через курьеров маркетплейса и склады до покупателя, с реальным мониторингом сроков выполнения операций и автоматическими мерами при нарушениях сроков. Использовать высоконагруженную архитектуру (highload), масштабируемую по городу/региону и объему событий. |
| Project Goals  | - Точная регистрация событий статусов (Принял/Отправил/взял/в работе/отдал) в журнал; - Выявление и автоматическое уведомление о нарушениях сроков; - Рабочее место Операционного контролера для оперативного реагирования; - Поддержка горизонтального масштабирования и наблюдаемости (мониторинг, SLA, логи).                                            |
| Target Market  | Маркетплейс с большим количеством городов, поставщиков, складов, курьеров и покупателей; поддержка параллельной обработки множества потоков событий.                                                                                                                                                                                                        |
| Timeline       | По повинности: минимально жизнеспособный выпуск через 3 месяца, далее ежемесячные релизы с улучшениями качества данных и высокой доступности. Объем проекта на стадии MVP — 6–9 месяцев.                                                                                                                                                                    |
| Stakeholders   | Продуктовый офис, Архитектура и SRE, Разработка (микросервисы-смежники), QA, Операционный контроль, Клиентская поддержка, Бизнес-аналитик.                                                                                                                                                                                                                  |

#### 1.2 User Personas

| Role ID | User Role                        | Demographics                               | Pain Points                                                                                        | Goals                                                               | Priority |
| ------- | -------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | -------- |
| UR-001  | Операционный контролер           | 30–45 лет, опыт в логистике 5–12 лет       | Нужно видеть реальное состояние цепочки в реальном времени; быстро реагировать на нарушения сроков | Быстро обнаруживать нарушения и инициировать меры                   | Must     |
| UR-002  | Складской оператор               | 25–50 лет, операционная работа с системами | Требуется точное и своевременное обновление статусов на складе                                     | Обновлять статусы без задержек, минимизировать ошибки               | Should   |
| UR-003  | Курьер                           | 20–50 лет, водитель/курьер                 | Нужно подтверждать начало и завершение доставки, фиксировать задержки                              | Передавать корректные статусы в журнал вовремя                      | Should   |
| UR-004  | Системный администратор / DevOps | 28–55 лет                                  | Ответственность за стабильность и масштабируемость                                                 | Поддерживать высокую доступность, быстрые развёртывания, мониторинг | Must     |
